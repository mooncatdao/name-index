...
import { enrichEventsWithBlockTimestamps } from "../src/timestamp-enrichment.js";
...
function parseArguments(argv) {
  ...
  return {
    eventsPath,
    eventsSource,
    ...(Object.hasOwn(values, "max-blocks")
      ? { maxBlocks: parsePositiveInteger(values["max-blocks"]) }
      : {}),
...
async function main() {
  ...
  const enrichment = await enrichEventsWithBlockTimestamps(
    createMoonCatPublicClient(rpcUrl),
    existingEvents,
    { maxBlocks: paths.maxBlocks }
  );
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    fetchedBlockCount: enrichment.fetchedBlockCount,
    enrichedBlockCount: enrichment.enrichedBlockCount,
    enrichedEventCount: enrichment.enrichedEventCount,
    remainingBlockCount: enrichment.remainingBlockCount,
    eventsChanged,
